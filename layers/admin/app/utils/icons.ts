export type IconName =
  ...
  | 'copy' | 'clipboard' | 'clipboard-paste' | 'more-horizontal' | 'info' | 'trash' | 'x' | 'plus' | 'pencil' | 'check'
  ...
  | 'clock' | 'history' | 'user' | 'users' | 'star' | 'activity' | 'lock'
...
export const icons: Record<IconName, string> = {
  ...
  'clipboard-paste': '<rect width="8" height="4" x="8" y="2" rx="1" ry="1"/><path d="M8 4H6a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h2.5"/><path d="M16 4h2a2 2 0 0 1 2 2v3.5"/><path d="M21 14H11"/><path d="m15 10-4 4 4 4"/>',
...
export function sanitizeIconSvg(svg: string): string {
  return svg.replace(/<(\/?)([a-zA-Z][\w:-]*)((?:"[^"]*"|'[^']*'|[^>])*?)(\/?)>/g, (_m: string, close: string, tag: string, attrs: string, selfClose: string) => {
    ...
    while ((a = attrRe.exec(attrs))) {
      const [, attrName, attrValue] = a
      if (attrName === undefined || attrValue === undefined) continue
      if (ALLOWED_SVG_ATTRS.has(attrName.toLowerCase())) kept.push(`${attrName}=${attrValue}`)
    }
